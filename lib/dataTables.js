...
TabularTables.NetworksList = new Tabular.Table({
  name: "NetworksList",
  collection: Networks,
  columns : [
      { data: 'name',  title: 'Name'  , render: function (val, type, doc) { ;
          return  "<a href='/networks/"+doc._id +"''>" + val +"</a>"; 
        } 
      },
      { data: 'sharedPublic',  title: 'Public', render: function (val, type, doc) { 
          return val ? "Public" : "Private";
        }
      },
      { data: 'createdAt',  title: 'Created At', render: function (val, type, doc) { 
